...
class SparkETLJob:
    ...
    def run(self) -> None:
        ...
        # 1) Read source (with optional watermark filter)
        ...
        # 5) Write / Merge (write | merge_scd1 | scd2)
        self._write_by_strategy(df_out)

        # 6) POST DQ must validate persisted target
        target_df = self._read_target_df()
        post_summary = self._run_dq(stage="post", df=target_df, source_df=df_in)

        # 7) Publish guard: block if POST fails (after successful write)
        guard = self.config.dq.publish_guard if getattr(self.config, "dq", None) else None
        if guard and guard.enabled and guard.block_on_post_fail:
    ...
    def _read_source(self) -> DataFrame:
        ...
        reader = self.spark.read.format(s.format)
        for k, v in (s.options or {}).items():
            reader = reader.option(k, v)

        df = reader.load(s.path)

        # Watermark filter (incremental ingestion) applies to SOURCE read
    ...
    def _apply_transform(self, df: DataFrame) -> DataFrame:
        ...
        # You can pass params too if you want:
        # return fn(df, self.spark, {"job_name": self.config.job_name, **self.config.transform.params})
    ...
    def _run_dq(self, stage: str, df: DataFrame, source_df: DataFrame) -> Optional[DQRunSummary]:
        ...
        Returns DQRunSummary so run() can enforce publish_guard.
        """
